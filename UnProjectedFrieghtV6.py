...
if scatterPlot == "true":
   ...
   #     df_fltr1 = df_fltr1t[isLWLdf_fltr1]                 # df_fltr1 contains only the O-C pair data # Unsorted 
        ...
        df_fltr2['Count'] = np.arange(len(df_fltr2))         # add a column row index, Find how many rows for this O-C pair#print(df_fltr2.count())
        ...
        if len(df_sumCUBE) > 3:            
            ...
            df_in = df_sumCUBE.iloc[:, 8].values.reshape(-1, 1)  # column 9-1 = 8 = "Count" # [:,0 ] is all the rows from the zeroth (first) column 
            ...
            df_in = df_in.iloc[:, 0].values.reshape(-1, 1)     # a "2-D" single column of inputs for input to regression                      
            ...
            clf_RandomForestRegressor = RandomForestRegressor(n_estimators=50, max_depth=5, min_samples_split=10, random_state=None,
                                                  oob_score="true", n_jobs=1, verbose=0)
            regr = clf_RandomForestRegressor.fit(df_in, df_outRF)
            
            # save each model           
            pkl_filename = "regr_model_" + str(iiiv) + ".pkl"  # Save to file in the current working directory                                 
            with open(pkl_filename, 'wb') as file:
                pickle.dump(regr, file)
                score = regr.score(df_in, df_out)  # The .score method of the LinearRegression returns the coefficient of determination R^2 of the prediction not the accuracy
            ...
            for x in range(10):
                dfMax = dfMax + 1 
                in_testtmp = np.append(in_testtmp, dfMax)       # in_testtmp = a list: 'array([304, 305, 306, 307, 308, 309, 310, 311, 312, 313, 314])'          

            # make them readabble by skn.linear_model.LinearRegression()
            df_in_testtmp = DataFrame(in_testtmp,columns=['x_test_values']) # a dataframe for input to regression
            df_in_test = df_in_testtmp.iloc[:, 0].values.reshape(-1, 1)     # a "2-D" single column of inputs for input to regression   #print(df_in_test)    
            ...
            plt.figure()
            plt.plot(x,y, '+', x, fit_fn(x), 'k', markersize=3)               # 'k' = black line
            plt.title("CUBE_PCT" + ' = ' + str("% .4e" %fit_fn[1]) + ' * ' + 'Count' + ' + ' + str("% .4e" %fit_fn[0]) )
